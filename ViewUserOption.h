...
namespace PBL2APP {
	...
	public:int IndexCounter = 1;
		  ...
		  ViewUserOption(Form^ tempForm)
		  {
			  ...
			  for (int i = 0; i < 100; i++)
			  {
				  if ((Data_Of_Gui[i].get_Book_Id() != "\0" && (Data_Of_Gui[i].get_Total_Copies() - Data_Of_Gui[i].get_Borrowed_Book_Library()) != 0))
					  createnewrow();
...
#pragma endregion
		public:void createnewrow()
		{
			if (AvailableBooks->Rows->Count <= 0)
			{
				...
				if ((Data_Of_Gui[0].get_Total_Copies()-Data_Of_Gui[0].get_Borrowed_Book_Library()) != 0)
				{
